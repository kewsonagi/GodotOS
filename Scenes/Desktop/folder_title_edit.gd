...
func trigger_rename() -> void:
	...
	if folder.eFileType != BaseFile.E_FILE_TYPE.FOLDER:
		...
		%"Folder Title".text = "[center]%s" % folder.szFileName
		...
		if folder.get_parent() is DesktopFileManager:
			...
			for file_manager: FileManagerWindow in get_tree().get_nodes_in_group("file_manager_window"):
				if file_manager.file_path == folder.szFilePath:
					file_manager.sort_folders()
		for text_editor in get_tree().get_nodes_in_group("text_editor_window"):
			if text_editor.file_path == "%s/%s" % [folder.szFilePath, old_folder_name]:
				text_editor.file_path = "%s/%s" % [folder.szFilePath, folder.szFileName]
			elif text_editor.file_path == old_folder_name: # In desktop
				text_editor.file_path = folder.szFileName 
		...
		for file_manager: FileManagerWindow in get_tree().get_nodes_in_group("file_manager_window"):
			if file_manager.file_path.begins_with(old_folder_path):
				file_manager.file_path = file_manager.file_path.replace(old_folder_path, folder.szFilePath)
				file_manager.reload_window("")
			elif file_manager.file_path == folder.szFilePath.trim_suffix("/%s" % folder.szFileName):
				file_manager.sort_folders()
